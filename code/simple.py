...
class Num:
    ...
    def discretize(self, other, conf):
        best = 1
        rest = 0
        xys = [(good, best) for good in self._vals] + \
              [(bad, rest) for bad in other._vals]
        bins = len(xys)**conf['bins'][1]

        n1 = len(self._vals)
        n2 = len(other._vals)
        iota = conf['cohen'][1]*(self._stdiv*n1 + other._stdiv*n2)/(n1 + n2)
...
class Sample:
    def __init__(self, config):
        ...
        self._xs = []
        self._conf = config
        # print(self._conf['bins'][1])

    def conf(self, key):
        return self._conf[key][1]
    ...
    def dist(self, row_idx1, row_idx2, dist_type="aha"):
        ...
        for c_idx, item in enumerate(self._col_info):
            ...
            ret += self._cols[c_idx].dist(self._rows[row_idx1][c_idx],
                                          ...
                                          dist_type)**self.conf('p')
        if num_items == 0:
            return 0
        return (ret/num_items)**(1/self.conf('p'))
    ...
    def faraway(self, row_idx, candidates=None):
        ...
        n_samples = min(len(candidates), self.conf('samples'))
        sel = random.sample(candidates, n_samples)
        pile = self.neighbors(row_idx, sel)
        return pile[math.floor(len(pile)*self.conf('far'))]
    ...
    def divs(self, loud=False):
        leafs = []
        threshold = len(self._rows)**self.conf('enough')
        self._divs(range(len(self._rows)), 0, leafs, threshold, loud)
        return leafs

    def _divs(self, rows, level, leafs, threshold):
        loud = self.conf('loud')
        if loud:
...
# this gave me subscript issues, so I'm doint something probably less efficient.
    ...
    def shuffle_param(self):
        keys = list(self._conf)
        ...
        if read[0] == int:
            # print("2")
            r = random.randint(read[2], read[3])
        elif read[0] == float:
            # print("3")
            r = random.uniform(read[2], read[3])
        self._conf[key] = (read[0], r, read[2],
    ...
    def spit_params(self):
        for key in self._conf:
            if self._conf[key][5]:
                print(str(key) + " is " + str(self._conf[key][1]))

class Fft:
    def __init__(self, sample, branch, branches, level=0):
        stop = 2*len(sample._rows)**sample.conf('bins')
        # stop = sample.conf('max')
        ...
        for x in sample._xs:
            # xbins = []
            for bin in best._cols[x].discretize(rest._cols[x], sample._conf):
                # Is this too much?
                bins.append(bin)
            # bins.append(xbins)
        if len(bins) == 0:
            ...
            print(sample._xs)
            print("best._cols[x]:")
            print(best._cols[x]._vals)
            print("rest._cols[x]:")
            print(rest._cols[x]._vals)
            return
        bestIdea  = sample.values("plan",    bins)[-1][1]
        worstIdea = sample.values("monitor", bins)[-1][1]
        # pre = "|.. "*level
        split = []
        if sample._conf['forest'][1]:
            split = [(1, 0, bestIdea), (0, 1, worstIdea)]
        else:
            split = [random.choice([(1, 0, bestIdea), (0, 1, worstIdea)])]
        for yes, no, idea in split:
            ...
            if len(tree._rows) > 0:
                branch1 += [Displ(at=idea.at, low=idea.low, high=idea.high,
                                  ...
                                  then=yvals, n=len(leaf._rows), dat=med)]
                if len(tree._rows) <= stop:
                    med = tree._rows[math.floor(len(tree._rows)/2)]
                    yvals = []
                    for c_idx in tree._ys:
                        yvals.append(med[c_idx])
                    branch1 += [Displ(type=no, txt="  ", then=yvals,
                                      n=len(tree._rows), dat=med)]
                    branches += [branch1]
                else:
                    Fft(tree, branch1, branches, level=level+1)
            else:
                branch1 += [Displ(type=no, txt="  ", then=yvals,
                                  n=len(leaf._rows), dat=med)]
                branches += [branch1]
